...
const contributionSchema = new mongoose.Schema({
    ...
    origin: {
        raw: { type: String, required: true }, // The address string
        placeId: { type: String }              // Google Place ID if available
    },
    destination: {
        raw: { type: String, required: true },
        placeId: { type: String }
    },
    vehicleType: { type: String, required: true, enum: ['bus', 'keke', 'bike'] },
    fareAmount: { type: Number, required: true },
    timeOfDay: { type: String, required: true, enum: ['morning', 'afternoon', 'evening', 'night'] },
    conditions: { type: [String], default: [] }, // e.g., ['rainy', 'traffic']
    notes: { type: String },
    timestamp: { type: Date, default: Date.now },
    userReputation: { type: Number, default: 0 }, // Snapshot of user reputation at the time of contribution
